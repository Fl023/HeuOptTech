...
class MWCCPGeneticAlgorithm:
    ...
    def run(self, population_size=10, mutation_rate=1.0, max_generations=100, linear_scaling=True,
        ...
        timeline = [{} for _ in range(max_generations + 1)]
        start_time = time.time()
        ...
        timeline[0]["time"] = time.time() - start_time
        timeline[0]["objective"] = self.C_max - self.best_fitness
        for generation in range(max_generations):
            ...
            timeline[generation+1]["time"] = time.time() - start_time
            timeline[generation+1]["objective"] = self.C_max - self.best_fitness
        ...
        return self.best_solution + 1 + self.num_u, self.best_fitness, self.C_max - self.best_fitness, timeline


if __name__ == "__main__":
    statistics = defaultdict(list)
    file = "tuning_instances/small/inst_50_4_00001"
    start = time.time()
    ...
    for i in range(num_runs):
        print("\nRun", i+1)
        start = time.time()
        res = ga.run(population_size=13, mutation_rate=1.0, max_generations=100, linear_scaling=True,
                     selection_pressure=2, frac_children=0.46477, frac_elites=0.095553)
        best_solution, best_fitness, best_objective, timeline = res
        end = time.time()
        statistics[file].append(timeline)
        print()
